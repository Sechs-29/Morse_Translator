'use strict'; //保存されたデータをロードし、変換します。
...
function conversion() {
  ...
  var ton = document.getElementById('input-ton').value || '・',
      ...
      space = document.getElementById('input-space').value || '  '; //*モールス信号では、日本語と英語の信号が被っているものがあります。よって、日本語と英語が使える変換器を自称するからには、辞書を２つ作らないといけません。
  ...
  var dictionary = document.getElementById('toggle-language-english').checked ? englishDictionary : japaneseDictionary; //このタブが閉じられ、そして再度開かれた時に保存されたデータをロードするために、データをセーブします。
  ...
  */
  ...
  if (dictionary == englishDictionary) var inputArray = new String(document.getElementById('input-morse').value).toLowerCase().split('');else var inputArray = new String(transJapanese(document.getElementById('input-morse').value)).split('');
  /*
  ...
  */
  ...
  for (var i = 0; i < inputArray.length; i++) {
    if (dictionary[inputArray[i]]) outputArray.push(dictionary[inputArray[i]]);else outputArray.push(inputArray[i]);
  ...
  */
  ...
  for (var i = 0; i < inputArray.length; i++) {
    ...
    if (inputArray[i].normalize('NFD')[1]) {
      if (inputArray[i].normalize('NFD')[1].charCodeAt(0) == 0x3099) outputArray.push('゛');else if (inputArray[i].normalize('NFD')[1].charCodeAt(0) == 0x309A) outputArray.push('゜');
    }
  }
...
function kanaToHira(text) {
  return text.replace(/[\u30A1-\u30FA]/g, function (ch) {
